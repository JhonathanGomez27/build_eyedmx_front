"use strict";
(self["webpackChunkmultikart"] = self["webpackChunkmultikart"] || []).push([["default-node_modules_ngx-mask_fesm2022_ngx-mask_mjs"],{
...
class NgxMaskApplierService {
  ...
  cb = () => {}) {
    ...
    if (maskExpression.startsWith("percent" /* MaskExpression.PERCENT */)) {
      ...
      } else if (result[position - 1] === this.decimalMarker || shiftStep === -4 || shiftStep === -3) {
        this._shift.clear();
      ...
      let i = 0, inputSymbol = inputArray[0]; i < inputArray.length; i++, inputSymbol = inputArray[i] ?? "" /* MaskExpression.EMPTY_STRING */) {
        ...
        if (this._checkSymbolMask(inputSymbol, maskExpression[cursor] ?? "" /* MaskExpression.EMPTY_STRING */) && maskExpression[cursor + 1] === "?" /* MaskExpression.SYMBOL_QUESTION */) {
          ...
          if (maskExpression[cursor] === "h" /* MaskExpression.HOUR */) {
            if (this.apm ? result.length === 1 && Number(result) > 1 || result === '1' && Number(inputSymbol) > 1 || inputValue.slice(cursor - 1, cursor).length === 1 && Number(inputValue.slice(cursor - 1, cursor)) > 1 || inputValue.slice(cursor - 1, cursor) === '1' && Number(inputSymbol) > 1 : result === '2' && Number(inputSymbol) > 3 || (result.slice(cursor - 2, cursor) === '2' || result.slice(cursor - 3, cursor) === '2' || result.slice(cursor - 4, cursor) === '2' || result.slice(cursor - 1, cursor) === '2') && Number(inputSymbol) > 3 && cursor > 10) {
              // eslint-disable-next-line no-param-reassign
              position = position + 1;
              cursor += 1;
              i--;
...
class NgxMaskService extends NgxMaskApplierService {
  ...
  formControlResult(inputValue) {
    if (this.writingValue || !this.triggerOnMaskChange && this.maskChanged) {
      this.maskChanged ? this.onChange(this._toNumber(this._checkSymbols(this._removeSuffix(this._removePrefix(inputValue))))) : '';
      this.maskChanged = false;
      return;
    }
    ...
    if (value && value.toString().length >= 1) {
      ...
      if (this._maskValue.indexOf("*" /* MaskExpression.SYMBOL_STAR */) === -1 || this._maskValue.indexOf("?" /* MaskExpression.SYMBOL_QUESTION */) === -1) {
        ...
        if (array.length > 1) {
          ...
          if (lastIndexArray && this._maskService.specialCharacters.includes(lastIndexArray[0]) && value.includes(lastIndexArray[0]) && !this.dropSpecialCharacters) {
    ...
    return (0,C_Smartdatacontact_Eyed_Eyed_node_modules_babel_runtime_helpers_esm_asyncToGenerator_js__WEBPACK_IMPORTED_MODULE_0__["default"])(function* () {
      ...
      if (typeof inputValue === 'number' || _this._maskValue.startsWith("separator" /* MaskExpression.SEPARATOR */)) {
        ...
        if (_this._maskService.leadZero && inputValue && _this.maskExpression && _this.dropSpecialCharacters !== false) {
          // eslint-disable-next-line no-param-reassign
          inputValue = _this._maskService._checkPrecision(_this.maskExpression.toString(), inputValue);
        }
